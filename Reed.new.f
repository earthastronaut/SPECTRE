      subroutine reed (mode,pts,wl,disp,fname,obsname,object,npt,
     .                 xxmin,xxmax,voverc,arrayn,filestyle)
c*****read the spectrum file into the program and parse the header 
c     for information
c
c     added the ability to read MAKEE formatted fits into SPECTRE
c
...
c
      ...
      character*8 WV0name,WV4name
      character*69 WV0data,WV4data
c     added 'spectredisp' because otherwise it tries to use the WV_0_ header info for the dispersion equation rather than spectre's version of dispersion.
...
1020        format('ERROR: CANT DO NAXIS1 =',i6,', NAXIS1 =',i6)
            ...
            nord = 1
            write (6,2021)
 2021       format("NOTE: maximumn number of points = 10,000")
            message = 
     ...
     .          waveiraf.eq.0) then
            ...
            if (naxhi-naxlo+1 .gt. MaxNpts) then
...
c**** MAKEE PIPELINE FOR KECK HIRES
c     Author: Dylan Gregersen


c     had to add this because otherwise it would crash with nord > 100
c     when trying to read the name
      if (nord .ge. 100) then
         write (errmess,2004)
         nchars=59
         call puterr (nchars)
      ...
      write(WV0name,2000) "WV_0_",nord
      write(WV4name,2000) "WV_4_",nord

   
      if (spectredisp .eq. 0) then
 192     do 151 j=1,nlines
            k = 80*(j-1)
            if (head(k+1:k+8) .eq. WV0name) then
               waveiraf = -2
               disp(9) = 4
c              check the wavelength coefficients
c               write (6,*) "------------"
c               write (6,*) "<",head(k+12:k+28),">"
c               write (6,*) "<",head(k+29:k+45),">"
c               write (6,*) "<",head(k+46:k+62),">"
c               write (6,*) "<",head(k+63:k+79),">"
c               write (6,*) "------------"
               
               write (WV0data,2001) head(k+12:k+79)
               read (head(k+12:k+28),2003) disp(1)
               read (head(k+29:k+45),2003) disp(2)
               read (head(k+46:k+62),2003) disp(3)
               read (head(k+63:k+79),2003) disp(4)
               write (WV0data,2001) head(k+12:k+79)
               
               if (head(k+81:k+88) .ne. WV4name) then
                  write(6,2002)
                  stop
                  ...
                  write (WV0data,2001) head(k+12:k+79)
c                  write (6,*) "------------"
c                  write (6,*) "<",head(k+80+12:k+80+28),">"
c                  write (6,*) "<",head(k+80+29:k+80+45),">"
c                  write (6,*) "<",head(k+80+46:k+80+62),">"
c                  write (6,*) "<",head(k+80+63:k+80+79),">"
c                  write (6,*) "------------"
                  write (WV4data,2001) head(k+92:k+159)
                  read (head(k+80+12:k+80+28),2003) disp(5)
                  read (head(k+80+29:k+80+45),2003) disp(6)
                  read (head(k+80+46:k+80+62),2003) disp(7)
                  read (head(k+80+63:k+80+79),2003) disp(8)                 
               endif
            endif
 151     continue
      endif

c     if get a run time error here it's probably related to the 'read (head(k+12:k+28),2003) disp(1)' lines below 
c     check the FITs file, it may be formatted differently than expected, 
c     it expects: 'WV_0_01 = '  4.017457233E+03  1.870226734E-02 -1.938077385E-07 -2.478301965E-11'WV_4_01 = ' -1.225285944E-15  1.051121502E-19  8.366220351E-24  0.000000000E+00'



 2000 format(A,i2.2)      
 2001 format(A)
 2002 format("ERROR:VALUE FOLLOWING <",
     .     A,"> was unexpected")
 2003 format(E30.10) 
 2004 format('ERROR: order number too large ',
     .     'for MAKEE wavelength solution')
...
c$$$$
c$$$$  This is where adjust to give more options:
c$$$$  waveiraf = -1, -2

      if (waveiraf .eq. -2) then
         go to 148
      endif
      if (waveiraf .eq. -1) then
         go to 130
      endif
      if (waveiraf.ge.0 .and. apline.eq.0) then
         go to 146
      endif
      if (oned .eq. 0) then
         go to 146
...
c      write (*,*) "<",disp(9),">"
      ...
      if (disp(2).ne.1.0 .and. wl(npt).lt.wl(1)) then
       ...
       call minimax(wl,xxmin,xxmax,npt)
...
C          endif
C          if (i .eq. 2) then
C             disp(i)=dble((xxmax-xxmin)/dble(npt))
C          endif
C          disp(i) = 0.
C       enddo
