...
c     this is the is version 2.2.1 but with some error checking stuff added
c*****read the spectrum file into the program and parse the header 
c     for information

c added some error checking stuff. 
c added:
c include 'Dataval.com'
c write (*,*) 'heyx ....etc
...
1020        format('ERROR: CANT DO NAXIS1 =',i6,', NAXIS1 =',i6)
     ...
     .        'LONG SPECTRUM: NAME UPPER WAVELENGTH/POINT LIMIT: '
            ...
            call getnum (nchars,xnum)
            wavhi = xnum
c            if (crval1.eq.0.0 .and. crpix1.eq.0.0 .and. cdelt1.eq.0.0
c     .          .and. w0.eq.0.0 .and. wpc.eq.0.0) go to 130
            if (crval1.eq.0.0 .and. crpix1.eq.0.0 .and. cdelt1.eq.0.0
     .          .and. w0.eq.0.0 .and. wpc.eq.0.0) then
               
               write (*,*) 'heyx skip 301 cc'
               go to 130
            endif
     ...
     .          waveiraf.eq.0) then
      ...
      write (*,*) 'heyx6',wlx(1),wlx(100),wlx(npx)
...
110         call getnum (nchars,xnum)
      ...
      write (*,*) 'heyx7',wlx(1),wlx(100),wlx(npx)
...
111         call getnum (nchars,xnum)
      ...
      write (*,*) 'heyx7a',wlx(1),wlx(100),wlx(npx)
...
c*****beginning and ending points within the data array
      ...
      write (*,*) 'heyx7b',wlx(1),wlx(100),wlx(npx)
...
c*****initialize the arrays
      do i=1,10000
         ...
         pts(i) = 0.
      enddo
      write (*,*) 'heyx this sucks'
      wl(1) = -9999.
      
      write (*,*) 'heyx7.1',wlx(1),wlx(100),wlx(npx)
      write (*,*) 'heyx test',wl(1),wl(2000),pts(5000)
...
148   if (disp(2) .eq. 0.) disp(2) = 1.
     ...
     .   disp(9).eq.0.0) then
      ...
      write (*,*) 'heyx9',wlx(1),wlx(100),wlx(npx)
