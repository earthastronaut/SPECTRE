...
*     DECK POLFIT
      SUBROUTINE polfit (N, XX, YY, W, MAXDEG, NDEG, EPS, R, IERR, A)
...
C     Abstract
C
C     Given a collection of points XX(I) and a set of values Y(I) which
C     correspond to some function or measurement at each of the XX(I),
...
C     express the polynomial fits as powers of (XX-C) for any specified
C     point C.
C
C     The parameters for  POLFIT  are
C
C     Input --
C         N -      the number of data points.  The arrays XX, YY and W
C                  must be dimensioned at least  N  (N .GE. 1).
C         XX -      array of values of the independent variable.  These
C                  values may appear in any order and need not all be
C                  distinct.
C         YY -      array of corresponding function values.
...
C                  user should set the weights to:  W(I) = 1.0/YY(I)**2,
C                  I = 1,...,N .
...
C                  of the fit of degree  NDEG  at each of the  XX(I) .
...
C
 13   TEMD1 = 0.0D0
      DO 14 I = 1,M
        K4PI = K4 + I
        TEMD1 = TEMD1 + DBLE(W(I))*DBLE(YY(I))*DBLE(A(K4PI))
 14     CONTINUE
 ...
 15     SIGJ = SIGJ + W(I)*((YY(I)-R(I)) - A(K5PI))**2
      J = 0
C
...
C
      ...
      DO 18 I = 1,M
        K4PI = K4 + I
        TEMD2 = A(K4PI)
        TEMD1 = TEMD1 + DBLE(XX(I))*DBLE(W(I))*TEMD2*TEMD2
 18     CONTINUE
...
C
      ...
      DO 19 I = 1,M
        ...
        TEMP = A(K3PI)
        A(K3PI) = A(K4PI)
        A(K4PI) = (XX(I)-A(JP1))*A(K3PI) - A(K1PJ)*TEMP
...
C
      ...
      DO 20 I = 1,M
        K4PI = K4 + I
        K5PI = K5 + I
        TEMD2 = DBLE(W(I))*DBLE((YY(I)-R(I))-A(K5PI))*DBLE(A(K4PI))
...
C
      ...
      DO 21 I = 1,M
        ...
        R(I) = TEMD2
        A(K5PI) = TEMD2 - DBLE(R(I))
 21     SIGJ = SIGJ + W(I)*((YY(I)-R(I)) - A(K5PI))**2
C
...
C
      ...
      NDER = 0
      DO 35 I = 1,M
        CALL PVALUE (NDEG,NDER,XX(I),R(I),YP,A)
 35     CONTINUE
 36   EPS = SQRT(SIG/XM)
 37   RETURN
      END
...
*DECK PVALUE
      SUBROUTINE PVALUE (L, NDER, XX, YFIT, YP, A)
...
C         XX -      the argument at which the polynomial and its
...
C
   11 WRITE (XERN1, '(I8)') L
      WRITE (XERN2, '(I8)') NORD
c     I took out XERMSG because it required many more subroutines
...
c     *   ', COMPUTED BY POLFIT -- EXECUTION TERMINATED.', 8, 2)
      write (6,*) 'COMPUTED BY POLFIT -- EXECUTION TERMINATED.'
      RETURN
C
...
c     +   'REQUESTED IS NEGATIVE -- EXECUTION TERMINATED.', 2, 2)
 12   write (6,*) 'INVALID INPUT PARAMETER. '//
     $     'ORDER OF POLYNOMIAL EVALUATION ' //
