...
c     Programmers: Michael Fitzpatrick, Chris Sneden and Dylan Gregersen
c     Programming begun March 1, 1985 ---- ended....sometime in the future
c
c     Please see Version.txt for most detailed information about changes
c
c     x, y, and z are the three main spectrum arrays. 
 
c***** Declare variables to include
      include 'Dataval.com'
      include 'Datachr.com'
      include 'Chars.com'
      include 'Plotchr.com'
      include 'Plotval.com'
      logical nogo,splitd
      character arr*1,arr1*1,arr2*1,nofile*80


      integer ndeg
      real*8 APOLY(15000,15000)
c      real*8 polyarray(15000,15000)
...
c      codepath = "/uufs/astro.utah.edu/common/astro_data/sp/"
...
c*****mc = have a command run on multiple input files
      elseif (command .eq. 'mc') then
            splitd = .false.
            call multicmd
            go to 10
...
c*****pf = conduct a polynomial fit on the data wavelength solution
c     added with Polyfit
         elseif (command .eq. 'pf') then

c****** - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
c           If you get this working then uncomment this line
            write (errmess,9941)
 9941       format("Currently no available")
            nchars = 22
            call puterr (nchars)
            go to 10
c****** - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 

 9940       format("<",f8.3,"><",f8.3,">")
 9931       call which (arr)
            if (arr.eq.'x' .or. arr.eq.'X') then
               write (6,*) "-------------------------"
               write (6,9940) 
               write (6,9940) dispx(1),dispx(2)
               write (6,9940) dispx(3),dispx(4)
               write (6,9940) dispx(5),dispx(6)
               write (6,9940) dispx(7),dispx(8)
               write (6,9940) dispx(9),0.0
               write (*,*) "---------- pre --------"
               pause
c               call polfit (npx,wlx,x,-1.0,7,ndeg,3,dispx,istat,APOLY)
               write (*,*) "-------------------------"
               write (6,9940) dispx(1),dispx(2)
               write (6,9940) dispx(3),dispx(4)
               write (6,9940) dispx(5),dispx(6)
...
c     call polyfit (wly,y,npy,6,dispy)
               continue
            else if (arr.eq.'z' .or. arr.eq.'Z') then
               continue
c     call polyfit (wlz,z,npz,6,dispz)
            else
               go to 9931
            endif
            call screenstat (1)
...
4031        call which (arr)
            if (arr.eq.'x' .or. arr.eq.'X') then
                   call rebin (wlx,x,dispx,npx,xfile)
            else if (arr.eq.'y' .or. arr.eq.'Y') then
                   call rebin (wly,y,dispy,npy,yfile)
            else if (arr.eq.'z' .or. arr.eq.'Z') then
                   call rebin (wlz,z,dispz,npz,zfile)
            else
                   go to 4031
            endif
            call screenstat (1)
...
4033        call which (arr)
            if (arr.eq.'x'.or.arr.eq.'X') then
                       call reed (1,x,wlx,dispx,xfname,xkfnam,xobj,npx,
     .                       xmin,xmax,vovercx,xary,xfile)
                 ...
                 call reed (1,y,wly,dispy,yfname,ykfnam,yobj,npy,
     .                      dummy1,dummy2,vovercy,yary,yfile)
                 call screenstat (1)
            else if (arr.eq.'z'.or.arr.eq.'Z') then
                 call reed (1,z,wlz,dispz,zfname,zkfnam,zobj,npz,
...
4007        call which (arr)
            if (arr.eq.'x' .or. arr.eq.'X') then
               if (xfile .eq. 'text') then
                  ...
                  call save (0,x,wlx,npx,dispx,xfname,nofile,xary,xobj)
               endif
            else if (arr.eq.'y'.or.arr.eq.'Y') then
               if (yfile .eq. 'text') then
                  call stext (0,wly,y,npy,yfname,yobj)
               else 
                  call save (0,y,wly,npy,dispy,yfname,nofile,yary,yobj)
               endif
            else if (arr.eq.'z'.or.arr.eq.'Z') then
               if (zfile .eq. 'text') then
                  call stext (0,wlz,z,npz,zfname,zobj)
               else
                  call save (0,z,wlz,npz,dispz,zfname,nofile,zary,zobj)
               endif
            else
               go to 4007
            endif
...
4011        call which (arr)
            if (arr.eq.'x' .or. arr.eq.'X') then
               if (xfile .eq. 'text') then
                  ...
                  call save (1,x,wlx,npx,dispx,xfname,nofile,xary,xobj)
               endif
            else if (arr.eq.'y'.or.arr.eq.'Y') then
               if (yfile .eq. 'text') then
                  call stext (1,wly,y,npy,yfname,yobj)
               else
                  call save (1,y,wly,npy,dispy,yfname,nofile,yary,yobj)
               endif
            else if (arr.eq.'z'.or.arr.eq.'Z') then
               if (zfile .eq. 'text') then
                  call stext (1,wlz,z,npz,zfname,zobj)
               else
                  call save (1,z,wlz,npz,dispz,zfname,nofile,zary,zobj)
               endif
            else
               go to 4011
            endif
...
c*****xy = exchange the x and y spectrum arrays
      elseif (command .eq. 'xy' .or. command .eq. 'yx') then
...
c*****xz = exchange the x and z spectrum arrays
      elseif (command .eq. 'xz' .or. command .eq. 'zx') then
...
c*****yz = exchange the y and z spectrum arrays
      elseif (command .eq. 'yz' .or. command .eq. 'zy') then
