      subroutine reedauto (pts,wl,disp,fname,obsname,object,npt,
     .                 xxmin,xxmax,voverc,arrayn,filestyle)
c*****read the spectrum file into the program and parse the header 
c     for information
c
c###########################################################
c  Edited by Dylan Gregersen
c
c  This is at core same as Reed.redhat.f used in big spectre
c  however several lines were removed (L57) so that the variable
c  fname is used and the filename it opens
c
c############################################################
...
c*****Get the file name and open the file
      call blank
c      message = 'ENTER THE FILENAME: '
c      nchars =  20
c      call getasci (nchars)
      charstat = 'old    '
      iunit = 8
c      fname = array(1:80)
